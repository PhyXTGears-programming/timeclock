...
def calculate(pt=False):
	...
	with open("usernameFile.txt", 'r') as nameFile:  # Open the file that contains all student names (file name specified in Settings)
		...
		for i in range(tempList.count("")):
			tempList.remove("")  # Remove the extra space(s) from the end
	tempList.sort()  # Sort
				...
				if linenum + 1 < len(datalist):  # If this is not the last line in the file:
					...
					if Settings['short']:
						...
						for i in range(2 - len(month)):
							month = "0" + month  # Add leading zeros
						day = str(signIn[13])
					...
					if datalist[linenum].split()[1] == "in" and datalist[linenum + 1].split()[1] == "out":  # If this line and the next are a valid in-out sequence
						# Calculate time:
						if signIn[7] == signOut[7]:
							time = int(signOut[3]) - int(signIn[3])  # If same day, subtract the in time from the out time to get the minutes in between
						else:  # Different Day (past midnight)
							time = 1440 - int(signIn[3]) + int(signOut[3])  # (1440 minutes = 24 hours) (1440-in time)=minutes from first day + out time = minutes from both days
							for i in range(int(signOut[7]) - int(signIn[7]) - 1):
								time += 1440  # For each extra day (double all-nighter), add 1440 minutes
						...
						if date not in dateDict[name]:
							...
							if dateDict[name][date] == 'Forgot':
								dateDict[name][date] = time  # If forgot there, replace that with time
							else:
								dateDict[name][date] += time  # Else, add the time to the time already there.
						...
						if date not in dateDict[name]:
							dateDict[name][date] = "Forgot"  # Set time for that date to 'forgot'
		except:
			if(pt):
...
def writeCSV():
	...
	while True:
		...
		try:
			...
			print("Close the file before overwriting it.")
